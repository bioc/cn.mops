...
%The underlying assumptions of the normalization procedure is that the ploidy
%of each sample is the same. If the default normalization procedure
%is applied to heterosomes or tumor samples, \cnmops\ will not produce the expected
%results. A normalization step that takes into account these ploidies per
%chromosome is under development.
